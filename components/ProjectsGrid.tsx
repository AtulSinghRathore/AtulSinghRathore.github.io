...
export default function ProjectsGrid({ data }: { data: Project[] }) {
  ...
  const allTags = useMemo(() => {
    ...
    for (const p of data) for (const t of (p as any).tags ?? []) set.add(t);
  ...
  const shown =
    tag === "All" ? data : data.filter((p) => ((p as any).tags ?? []).includes(tag));
        ...
        <AnimatePresence initial={false}>
          {shown.map((p) => {
            const cover: string | undefined =
              "cover" in p ? ((p as any).cover as string | undefined) : undefined;
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="mt-2 flex gap-1.5 flex-wrap">
                    {((p as any).tags ?? []).map((t: string) => (
                      <span
                        key={t}
                        className="text-xs text-muted border border-[#2b3243] px-2 py-0.5 rounded-full"
                      >
                        {t}
                      </span>
                    ))}
